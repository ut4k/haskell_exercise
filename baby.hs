...
-----------------------------------------------------
-- $を使った関数適用
-- カッコの中身を最後に計算する用の開きかっこの始まりみたいなもの
-----------------------------------------------------

-- sqrt 3 + 4 + 9  -----> これは3の平方根+4+9になる
-- でも本当は3+4+9の平方根が欲しい!
-- squrt $ 3 + 4 + 9
-- こうすると$は最後に関数適用されるので 3 + 4 + 9が適用されてからsqrtがその結果に適用される
--
-- 他にもカッコだらけの関数適用を見やすくする効果もアリ
-- 右結合　の意
--
-----------------------------------------------------
-- 関数合成
-- 他の関数に渡す関数をその場で作るときに使う
-- 
-- 一度にたくさんの関数も合成できる
-----------------------------------------------------

-- リストの数をすべて負の数にする関数をマップしたいとき
-- absで絶対値ををとってからそれをnegateで負の数にする
--
-- ラムダ式で作ってる無名関数
-- map (\x -> negate (abs x)) [5,-3,-6,7,-3,2,-19,24]
-- result : [-5,-3,-6,-7,-3,-2,-19,-24]
--
-- これは関数合成にすると...
--
...
--
-- ------------------
-- リストのtail(先頭のぞく後ろ全部)の合計を負の数にする関数　を
-- リスト内のリスト１つずつに適用していく
